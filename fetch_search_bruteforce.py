...
# Generar lista de nuevo porque el JSON se sobreescribió a vacío en algún paso
...
print("Extracción Fuerza Bruta (Search Exacto por Archivo)...")
success = 0
errors = 0

for idx, page_name in enumerate(page_names[:30]):
    print(f"[{idx+1}/30] {page_name}")
    ...
    try:
        with urllib.request.urlopen(req_search) as response:
            ...
            if not hits:
                print(f"  -> No encontrada en Search.")
                errors += 1
                continue
