...
with open('/Users/usuario2/Documents/eatcloud-docs-framework/missing_pages.json', 'r', encoding='utf-8') as f:
...
print(f"Iniciando descarga masiva de {len(missing_pages)} páginas...")
success_count = 0
error_count = 0

for idx, page_name in enumerate(missing_pages):
    print(f"[{idx+1}/{len(missing_pages)}] Procesando: {page_name}")
    ...
    try:
        with urllib.request.urlopen(req_search) as response:
            ...
            if not hits:
                print(f"  -> No encontrada vía Search API.")
                error_count += 1
                continue
...
print(f"\nFinalizado. Exitos: {success_count}, Errores: {error_count}")
